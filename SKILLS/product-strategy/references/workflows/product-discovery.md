...
2. Separate evidence from assumptions. Mark each assumption as safe, risky, or must-confirm; use [discovery-brief.md](../artifacts/discovery-brief.md) when a formal brief or assumption table is needed.
...
- If discovery depends on market norms or competitor behavior, hand that research to `market-researcher` using $product-strategy in `competitive-research` mode.
