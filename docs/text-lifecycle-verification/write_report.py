...
import json
R=Path('/srv/data/code/codex-subagents');D=R/'docs/text-lifecycle-verification';data=json.loads((D/'live-results.json').read_text())
s='''# Text Lifecycle Verification
...
'''
for c in data['cases']:
 s+=f"| [{c['case']}](text-lifecycle-verification/cases/{c['case']}/response.md) | {c['note']} |\n"
s+='''
...
'''
(R/'docs/text-lifecycle-verification.md').write_text(s)
print('Wrote verification record with',len(data['cases']),'cases')
